...
var upOrDown =  function(current) {
  var possibility = Math.floor(Math.random() * Math.floor(2));
  var price = Faker.finance.amount(0, 5.00);
  var result;
...
const fakeMassiveData =[];

for (let i = 1; i <= 10000000; i++) {
  const random = Math.floor(Math.random() * 100)
  const price = Faker.finance.amount(10.00, 400.00)
  fakeMassiveData.push({
    id: i,
    ticker: tickers[i - 1],
    company: companyData[random].company,
    currentPrice: price,
